...
function config(){
  ...
  ancho2 = ancho_total*0.5 - margins.left - margins.right
  alto2  = alto_total*0.5 - margins.top - margins.bottom
  ...
  xAxisGroup2 = g2.append('g')
                .attr('transform', `translate(0, ${ alto2 })`)
                .attr('class', 'ejeX2')
  yAxisGroup2 = g2.append('g')
                .attr('class', 'ejeX2')
...
function frame2(Anio){
  ...
  y2.domain([0, 18000000])
  x2.domain(dataframe.map(d => d.Estado))
...
function render2(data) {
  ...
  bars2 = g2.selectAll('rect')
            .data(data2, d => d.Estado)
  ...
  bars2.enter()
      ...
      .merge(bars2)
        ...
        .duration(2000)
          .style('x', d => x2(d.Estado) + 'px')
          .style('y', d => (y2(d["Poblacion"])) + 'px')
          .style('height', d => (alto2 - y2(d["Poblacion"])) + 'px')
          .style('fill', d => color(d.Estado))
          .style('width', d => `${x2.bandwidth()}px`)
  ...
  xAxisCall2 = d3.axisBottom(x2)
  xAxisGroup2.transition()
            .duration(2000)
            .call(xAxisCall2)
            .selectAll('text')
            .attr('x', '-8px')
